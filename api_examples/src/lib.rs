...
pub extern "C" fn pick_row(object: K, index: K) -> K {
    match object.get_type() {
        qtype::TABLE => match object.get_row(index.get_long().unwrap() as usize, &["sym"]) {
            Ok(row) => row,
            Err(error) => new_error(error),
        },
...
pub extern "C" fn thai_kick(_: K) -> K {
    unsafe {
        krr(null_terminated_str_to_const_S(
            "Thai kick unconditionally!!\0",
        ))
    }
}
...
pub extern "C" fn create_simple_list(_: K) -> K {
    ...
    for i in 0..5 {
        ...
        unsafe {
            ja(
                &mut list,
                std::mem::transmute::<*mut J, *mut V>(&mut timestamp),
            );
        }
    }
    list
...
pub extern "C" fn create_compound_list(_: K) -> K {
    unsafe {
        ...
        jk(
            &mut list,
            kpn(str_to_S!("3rd"), "3rd".chars().count() as i64),
        );
        list
    }
}
...
extern "C" fn counter(socket: I) -> K {
    ...
    unsafe {
        send(
            socket,
            message.as_slice().as_ptr() as *const c_void,
            total_length,
            0,
        )
    };
...
pub extern "C" fn plumber(_: K) -> K {
    ...
    let handle = std::thread::spawn(move || {
        ...
        unsafe {
            libc::write(
                PIPE[1],
                std::mem::transmute::<*mut K, *mut V>(&mut precious),
                8,
            )
        };
    });
...
impl Planet {
    ...
    fn description(&self) -> String {
        let mut desc = format!(
            "The planet {} is a beautiful planet where {} people reside.",
...
pub extern "C" fn probe(planet: K) -> K {
    ...
    unsafe {
        native::k(
            0,
            str_to_S!("{[func; planet] func[planet]}"),
            load_as_q_function(invade as *const V, 2),
            planet,
            KNULL,
        )
    }
